...
class VisualFeature:
    ...
    def __post_init__(self) -> None:
        ...
        if not np.isfinite(pixel).all() or self.area <= 0:
            raise ValueError("visual feature must have a finite pixel and positive area")
...
class ColorBlobDetector:
    ...
    def __init__(self, target_rgb: tuple[int, int, int] = (220, 60, 45), tolerance: float = 90.0,
                 min_area: int = 8) -> None:
    ...
    def detect(self, image: ImageFrame | np.ndarray) -> VisualFeature | None:
        ...
        pixel = np.array([np.average(xs, weights=weights), np.average(ys, weights=weights)])
        confidence = float(np.clip(1.0 - np.average(distance[ys, xs]) / self.tolerance, 0.0, 1.0))
...
class CameraCalibration:
    ...
    def __post_init__(self) -> None:
        ...
        translation = np.asarray(self.translation_base_camera, dtype=np.float64).reshape(3)
        if min(self.fx, self.fy) <= 0 or not np.isfinite(rotation).all() or not np.isfinite(translation).all():
    ...
    def ray_base(self, pixel: np.ndarray) -> np.ndarray:
        ...
        ray_camera = np.array([(pixel[0] - self.cx) / self.fx, (pixel[1] - self.cy) / self.fy, 1.0])
...
class SO101VisualServoPolicy(Policy):
    ...
    def __init__(self, env, *, camera: str = "front", detector: ColorBlobDetector | None = None,
                 calibration: CameraCalibration | None = None, target_pixel: tuple[float, float] | None = None,
                 target_plane_z: float = 0.035, kp: float = 2.0, max_speed: float = 0.08,
                 pixel_tolerance: float = 8.0, ee_tolerance: float = 0.012,
                 max_joint_rate: float = 1.2, dt: float | None = None,
                 final_camera: str = "wrist") -> None:
        self.env = env
        self.camera = camera
        ...
        self._resolver = TwistToJointResolver(env.kin, data=env.data, dt=control_dt, max_joint_step=0.08)
    ...
    def _calibration_from_env(self, camera: str | None = None) -> CameraCalibration:
        ...
        fy = height / (2.0 * np.tan(np.deg2rad(self.env.model.cam_fovy[camera_id]) / 2.0))
    ...
    def _waypoint(self) -> tuple[np.ndarray, float]:
        ...
        if self._phase in (VisualServoPhase.DESCEND, VisualServoPhase.CLOSE):
            return np.array([*self._target_xy, cube_z]), 1.0 if self._phase is VisualServoPhase.DESCEND else 0.19
        ...
        if self._phase is VisualServoPhase.TRANSFER:
            return np.array([self.env.target_pos[0], self.env.target_pos[1], rest_z + 0.035]), 0.19
        if self._phase in (VisualServoPhase.LOWER, VisualServoPhase.RELEASE):
            return np.array([self.env.target_pos[0], self.env.target_pos[1], rest_z + 0.016]), 0.19 if self._phase is VisualServoPhase.LOWER else 1.0
        return np.array([self.env.target_pos[0], self.env.target_pos[1], rest_z + 0.035]), 1.0

    def act(self, observation: Observation) -> Action:
        if self._target_xy is None:
            frame = observation.images.get(self.camera)
            if frame is None:
                self.metrics = VisualServoMetrics(failure_reason=f"missing_camera:{self.camera}")
                return Action(joint_position=self._q_cmd, gripper=GripperCommand(position=1.0))
            feature = self.detector.detect(frame)
            if feature is None:
                self.metrics = VisualServoMetrics(failure_reason="feature_not_found")
                return Action(joint_position=self._q_cmd, gripper=GripperCommand(position=1.0))
            try:
                self._target_xy = self.calibration.pixel_to_plane(feature.pixel, self.target_plane_z)[:2]
        ...
        self._grip = float(np.clip(grip_goal, self._grip - 0.9 * self._dt, self._grip + 0.9 * self._dt))
        ...
        return Action(joint_position=self._q_cmd, gripper=GripperCommand(position=self._grip))


__all__ = ["CameraCalibration", "ColorBlobDetector", "SO101VisualServoPolicy", "VisualFeature", "VisualServoMetrics"]